# https://github.com/JYWanng/LBCCN/blob/main/model.py
import torch
...
class Model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = self.rnn(x) # torch.Size([1, 2, 40, 126]) => torch.Size([1, 16, 126, 40])
        a = self.A_decoder(x) # torch.Size([1, 16, 126, 40]) => torch.Size([1, 40, 126])
        b = self.B_decoder(x) # torch.Size([1, 16, 126, 40]) => torch.Size([1, 40, 126])

        vrc = (xlc - b*xrc) / (a - b) # torch.Size([1, 40, 126])
        vlc = a*vrc # torch.Size([1, 40, 126])
        # nlc = xlc - vlc # torch.Size([1, 40, 126])
        # nrc = xrc - vrc # torch.Size([1, 40, 126])

        vl = torch.cat((vlc,xln),dim=1)
        vr = torch.cat((vrc,xrn),dim=1) #vlc.shape torch.Size([1, 40, 126])  xln.shape torch.Size([1, 89, 126]) vr = torch.cat((vlc,xln),dim=1).shape torch.Size([1, 129, 126])
        voice = torch.stack((self.istft(vl), self.istft(vr)),dim=1) # self.istft(vl).shape torch.Size([1, 16000]) voice.shape torch.Size([1, 2, 16000])
...
class Encoder(nn.Module):
    ...
    def forward(self, xlc, xrc, xln, xrn):
        ...
        xr_1 = xrc
        xr_2 = xrn
        convx_1 = self.conv_1_1(xl_1) # torch.Size([1, 40, 126]) => torch.Size([1, 40, 126])
        convx_2 = self.conv_1_2(xl_2) # torch.Size([1, 89, 126]) => torch.Size([1, 40, 126])
        xl = convx_1 + convx_2
    ...
    class Conv1dBlock(nn.Module):
        ...
        class ComplexPReLU(nn.Module):
            def __init__(self):
                super().__init__()
                self.r_prelu = nn.PReLU()  # Remove dtype specification
                self.i_prelu = nn.PReLU()

            def forward(self, input):
                # Force full precision for complex operations
...
if __name__ == '__main__':
    ...
    print(outputs[0].shape) # torch.Size([1, 2, 16000])

    # from thop import profile
    # from thop import clever_format
    # # from ptflops import get_model_complexity_info
    # # flops, params = get_model_complexity_info(model, (2,16000,), as_strings=True, print_per_layer_stat=True, verbose=True)
    # # print(flops, params)
